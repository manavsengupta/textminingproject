print("\n\nNote: Make Sure all the libraries are installed if not in cmd or Terminal write : pip install nltk pandas pdfminer.This Program is compatible with latest Python Version yet 3.8 .If in future the Program is not supported by latest python version or libraries consider using the virtualenv in my github repo for the Project : https://www.github.com/manavsengupta/textminingproj/ or installing the necessary libraries by the requirements.txt provided in the same github repo by running the command pip install -r requirements.txt and in case you are in linux and tkinter is not installed by default run sudo apt install python3-tk\n\n .....Loading.......")
import os
import pandas as pd
import nltk
nltk.download("punkt")
